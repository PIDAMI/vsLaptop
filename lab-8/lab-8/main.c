...
void PrintTree(node* root, int n) {
	if (root) {
		...
		PrintTree(root->third, n + 1);
		for (int i = 0; i < n; i++)
			printf(" ");

		PrintTree(root->second, n + 1);
		for (int i = 0; i < n; i++)
			printf(" ");
		...
		if (root->size == 1)
			...
			printf("%i %i\n", root->key[0], root->key[1]);

		PrintTree(root->first, n + 1);
	}
}
...
node* Search(node* p, int key) {
	...
	if ((p->size == 2 && key < p->key[1]) || p->size == 1)
...
void parse_command(char command, int key, node** root) {
	if (command == 'a') {
		if (!Search(*root, key)) {
...
int main() {
	...
	while ((command = getchar()) != EOF) {
		scanf("%i", &key);
		parse_command(command, key, &root);
	}
