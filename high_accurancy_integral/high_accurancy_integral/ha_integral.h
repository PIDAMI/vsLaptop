...
namespace gl {
	...
	const static double exact_val = (170755 * cos(4.0 / 5)) / 8 
									- (1913465 * cos(1.0 / 5)) / 64
									- (376101 * sin(1.0 / 5)) / 64
									+ (173823 * sin(4.0 / 5)) / 8;
	static long gauss_calls = 0;
	std::vector<double> nodes();
	std::vector<double> Y(const double a, const double b, const std::vector<double>& x);

	std::vector<double> YModCont(const double a, const double b,
								const std::vector<double>& x);
	std::vector<double> YModTail(const double a, const double b,
								const std::vector<double>& x);
	double elem_interval_integral(const std::vector<double>& y,
